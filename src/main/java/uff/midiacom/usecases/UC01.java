...
public class UC01 extends AbstractUseCase {
    ...
    private void generateReplayAttacksUC1(int res, String num, int numReplayInstances) throws IOException {
        ...
        if (printHeader) {
            if (!defaultHeader) {
                writeCustomHeader(columns, columns2, columnsGOOSEType);
            } else {
                writeDefaultHeader();
            }
            printHeader = false;
        ...
        for (int i = 0; i < numReplayInstances; i++) {
            ...
            String line = joinColumns(formatedCSVFile, formatedCSVFile2, columns, columns2, svIndex) + "," + gooseEventManager.getLastGooseFromSV(gooseTime).asCSVFull() + getConsistencyFeaturesAsCSV(gooseEventManager.getLastGooseFromSV(gooseTime)) + "," + label[1];
